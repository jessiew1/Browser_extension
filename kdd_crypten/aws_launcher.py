...
def upload_file(instance_id, client, localpath, remotepath):
    ...
    print(f"Uploading `{localpath}` to {instance_id} at {remotepath}...")
    sys.stdout.flush()
...
def main():
    ...
    print('to chmod is {:s}'.format(remote_script))
    print('Actually, I think it would be best to chmod {:s}'.format(remote_script.replace('\\', '/')))
    ...
    for instance_id, client in client_dict.items():
        remote_script2 = remote_script.replace('\\', '/')
        run_command(instance_id, client, f"chmod +x {remote_script2}")
