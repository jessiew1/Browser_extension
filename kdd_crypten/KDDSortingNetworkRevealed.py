...
"""
Exactly like KDDWithSortingNetwork.py except that when doing a comparison, all
compute parties learn which of the two elements is larger.

This file mainly exists to see if there is performance to be gained by letting
the selector bit in the following multiplexing operation be public.
"""
...
def compute_thresholds_enc(scores_enc, R, B):
    ...
    for r in range(len(B)):
        ...
        for a, b in wires:
            ...
            to_swap = to_swap.get_plain_text()
            if to_swap not in [0, 1]:
                raise RuntimeError('to_swap is of type {!s:s} and has value {!s:s}'.format(type(to_swap), to_swap))
            if to_swap == 1:
                scores_of_bag_enc[a] = vb
                scores_of_bag_enc[b] = va
